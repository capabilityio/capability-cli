...
exports.desc = "Media Service operations.";

exports.builder = function(yargs)
{
    const group = "Media:";
    return yargs.commandDir("media")
        .demandCommand()
        .option("capability",
            {
                group,
...
exports.capability = (args, name) =>
{
    ...
    else
    {
        ...
        if (config.capabilityExists(savedCredentials, args.profile, "media", name))
        {
            return savedCredentials[args.profile]["media"].capabilities[name];
        }
    }
    console.error("FAILED");
    console.error(`No media "${name}" capability found for profile "${args.profile}"`);
    process.exit(1);
};
